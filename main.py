    ...
    for i in range(markers):
        ...
        while True:
            ...
            if len(corners) > 0 and i in ids and len(ids) == 1:
                ...
                tello.go_xyz_speed(int(tvec[0][2]) - 100, int(0 - tvec[0][0]),
                                   int(0 - tvec[0][1] - 30), 50)
                rotate = int(rvec[0][0])
                if rotate > 0:
                    tello.rotate_clockwise(rotate)
                elif rotate < 0:
                    tello.rotate_counter_clockwise(0 - rotate)
                ...
                (corners, _, _) = detector.detectMarkers(image)
                rvec, tvec, _ = my_estimatePoseSingleMarkers(corners, markerSizeInCM, mtx, dist)
                tello.go_xyz_speed(int(tvec[0][2]) - 40, int(0 - tvec[0][0]),
                                   int(0 - tvec[0][1] - 30), 20)
                rotate = int(rvec[0][0])
                if rotate > 0:
                    tello.rotate_clockwise(rotate)
                elif rotate < 0:
